...
int mainLG(int argc, char **argv){
    if(argc != 3){
        cout << "Usage: ./LGSFolderDataSum YYYYMMDD y/n" << endl;
    }
    ...
    LGDataSummary ds(argv[1]);
    if(ds.hasData()){
        ds.t_disp->Print((outStr+"LG.pdf[").c_str());
        ...
        system(Form("chmod 660 %s",(outStr+"LG.pdf").c_str()));
        system(Form("chmod 660 %s",(outStr+"LG.csv").c_str()));
    }
    else{
